    ...
    for (const [addr, ids] of Object.entries(formattedSales)) {
      ...
      if (existingDoc) {
        ...
        if (newIds.length > 0) {
          ...
          if (db.TEST_MODE)
            console.log(`Inserted ${result.modifiedCount} into SUBS`);
        }
...
const subSalesBlur = async () => {
  ...
  try {
    while (true) {
        ...
        if (db.CATCHING_UP) {
          ...
          process.stdout.write(
            `\r\x1b[38;5;12m Current date:\x1b[0m ${currDate} ` +
              `\x1b[38;5;12m Memory usage:\x1b[0m ${(
                memoryUsage.rss /
                1024 /
                1024
              ).toFixed(2)} MB ` +
              `\x1b[38;5;12m Runtime:\x1b[0m ${(
                (Date.now() - db.CATCHING_UP_START) /
                1000
              ).toFixed(2)}} s `
          );

          if (currDate < db.CATCHING_UP_DATE + 60000) {
            //1m left
      ...
      process.stdout.write(
        `\r\x1b[38;5;202m AMT OF COLLECTED BLUR SALES:\x1b[0m ${(db.AMT_TOTAL_SALES +=
          newBlurSales.length)} `
      );
